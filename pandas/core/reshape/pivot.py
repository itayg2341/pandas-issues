...
def _add_margins(
    ...
    if not values and isinstance(table, ABCSeries):
        ...
        marginal_result_set = _generate_marginal_results_without_values(
            table, data, rows, cols, aggfunc, kwargs, observed, dropna=dropna, margins_name=margins_name
        )
        if not isinstance(marginal_result_set, tuple):
            return marginal_result_set
        result, margin_keys, row_margin = marginal_result_set
...
def _generate_marginal_results(
    ...
    if len(cols) > 0:
        row_margin = (
            data[cols + values].groupby(cols, observed=observed, dropna=dropna).agg(aggfunc, **kwargs)
        )
...
def _generate_marginal_results_without_values(
    ...
    if len(cols) > 0:
        ...
        if len(rows) > 0:
            ...
            margin = data.groupby(level=0, observed=observed, dropna=dropna).apply(aggfunc, **kwargs)
            all_key = _all_key()
            table[all_key] = margin
            result = table
            margin_keys.append(all_key)
